...
par(mfrow=c(1,2))
hist(ToothGrowth$len, breaks = 20, prob=TRUE)
lines(density(ToothGrowth$len), lty = 1)
lines(density(ToothGrowth$len, adjust = 1.5), lty = 2)
legend(x="topleft", lty = c(1,2), legend= c("Probability density", "Probability density smoothing kernel adjusted"))
plot.new()

qqline(ToothGrowth$len, datax = FALSE, distribution = qnorm,
       probs = c(0.25, 0.75), qtype = 7)
print("The distribution of length of tooth growth is slightly positively skewed")

library(RcmdrMisc)
ToothGrowth$dosecat <- recode(ToothGrowth$dose, "0:0.5='Low'; 0.51:1 = 'Med'; 1.1:2.0 = 'High'" )
ToothGrowth$dosecat <- as.factor
